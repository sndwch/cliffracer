...
def create_backdoor_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        'backdoor',
        help='Connect to a running service backdoor for debugging'
    )
    
    parser.add_argument(
        'connection',
        help='Connection string in format host:port (e.g., localhost:9999)'
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Connect to Cliffracer service backdoor'
    )
