...
class ServiceInspector:
    ...
    def _get_memory_usage(self) -> Optional[Dict[str, int]]:
    ...
    def print_summary(self):
        ...
        rpc_methods = [name for name, details in methods.items() 
...
def create_debug_context(service) -> Dict[str, Any]:
    ...
    context = {
        ...
        'nats_inspector': NATSInspector(getattr(service, 'nats', None)) if hasattr(service, 'nats') else None,
        
        # Utility functions
        'inspect_service': lambda: ServiceInspector(service).print_summary(),
        'inspect_nats': lambda: NATSInspector(getattr(service, 'nats', None)).print_summary() if hasattr(service, 'nats') else print("No NATS connection"),
