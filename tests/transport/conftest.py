"""The in-memory transport these tests run on, and its fixtures.

Nothing here dials a broker. MockJetStreamMsg and the transport below stand in
for NATS entirely, which is what the directory name says. The suite's
end-to-end coverage against a real broker lives in
...
class MockNatsTransport:
    """High-fidelity in-memory NATS transport for tests that never dial a broker."""
