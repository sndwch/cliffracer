...
class TestCorrelationDecorator:
    ...
    def test_sync_decorator(self):
        ...
        # Reset ambient correlation context to prevent leakage across tests.
        CorrelationContext.clear()
...
class TestServiceCorrelation:
    ...
    async def test_rpc_correlation_propagation(self):
        ...
        class TestService(CliffracerService):
            ...
            async def test_method(
                self, data: str, correlation_id: str = None
            ) -> dict[str, str | None]:
        ...
        await service.container._handle_rpc_request(mock_msg)
    ...
    async def test_rpc_correlation_generation(self):
        ...
        class TestService(CliffracerService):
            ...
            @rpc
            async def test_method(self, correlation_id: str = None) -> dict[str, str | None]:
    ...
    async def test_event_correlation_propagation(self):
        ...
        class TestService(CliffracerService):
            def __init__(self):
                super().__init__(config)
                self.container.registry.event_handlers["test.event"] = self.handle_test_event
        ...
        # Handle the event
        await service.container._handle_event(mock_msg)
...
async def test_end_to_end_correlation():
    ...
    class Chained(BaseModel):
        """service2_result is what Service2.process returns, so a dict of its own."""
    ...
    class Service1(CliffracerService):
        ...
        @rpc
        async def call_service2(self, correlation_id: str = None) -> Chained:
            # Call service2
            result = await self.call_rpc("service2", "process", data="test")
            return Chained(service1_correlation_id=correlation_id, service2_result=result)
