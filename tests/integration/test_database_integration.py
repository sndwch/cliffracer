...
class TestDatabaseIntegration:
    ...
    async def db(self):
        ...
        db = DatabaseConnection(
            ...
            user=os.getenv("TEST_DB_USER", "cliffracer"),
            password=os.getenv("TEST_DB_PASSWORD", "cliffracer123"),
            database=os.getenv("TEST_DB_NAME", "cliffracer_test"),
        )
    ...
    async def test_product_operations(self, db, setup_test_tables):
        ...
        # Find by criteria
        expensive_products = await product_repo.find_by(price=14.99)
        assert len(expensive_products) == 1
        assert expensive_products[0].product_id == "PROD_004"
