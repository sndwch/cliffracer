    ...
    async def setup(self):
        ...
        for queue_name in queues:
            ...
            queue_attributes = sqs.get_queue_attributes(
                QueueUrl=self.queue_urls[queue_name], AttributeNames=["QueueArn"]
            )
            queue_arn = queue_attributes["Attributes"]["QueueArn"]

            sns.subscribe(TopicArn=self.topic_arn, Protocol="sqs", Endpoint=queue_arn)
    ...
    async def seed_products(self):
        ...
        for product in products:
            ...
            inventory_table.put_item(
                Item={"product_id": product["product_id"], "stock": 100, "reserved": 0}
            )
    ...
    async def create_order(self, customer_id: str, items: list[OrderItem]) -> Order:
        ...
        cloudwatch.put_metric_data(
            ...
            MetricData=[
                ...
                {"MetricName": "OrderValue", "Value": float(total_amount), "Unit": "None"},
            ],
        )
        self.metrics_sent += 2
...
class PaymentService:
    ...
    async def process_payment(self, order_data):
        ...
        cloudwatch.put_metric_data(
            Namespace="Cliffracer/ECommerce",
            MetricData=[
                {"MetricName": "PaymentsProcessed", "Value": 1, "Unit": "Count"},
                {
...
class FulfillmentService:
    ...
    async def fulfill_order(self, order_data):
        ...
        cloudwatch.put_metric_data(
            Namespace="Cliffracer/ECommerce",
            MetricData=[{"MetricName": "OrdersFulfilled", "Value": 1, "Unit": "Count"}],
        )

        print(
            f"📦 Order fulfilled: {order_id} - Receipt stored in S3 (Tracking: {receipt['tracking_number']})"
        )
...
class MetricsCollector:
    ...
    async def show_metrics(self, services):
        ...
        try:
            ...
            print("🏗️  AWS Services: SNS, SQS, DynamoDB, S3, CloudWatch")
            print()
            ...
            print(
                f"   Inventory Service: {services['inventory'].processed_orders} orders processed"
            )
            print(
                f"   Payment Service: {services['payment'].processed_payments} payments processed"
            )
            print(
                f"   Fulfillment Service: {services['fulfillment'].fulfilled_orders} orders fulfilled"
            )
            print()

            print("🌐 AWS ENDPOINTS:")
            print("   LocalStack: http://localhost:4566")
            print("   CloudWatch Logs: http://localhost:4566/_aws/logs")
            print("   S3 Browser: http://localhost:4566/_aws/s3")
            print("=" * 70)
