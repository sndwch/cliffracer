...
Demonstrates scheduled background tasks using the @timer decorator.
"""
...
class HealthStatus(BaseModel):
    """What the health timer stores and get_health_status returns."""
...
class TimerStat(BaseModel):
    """One timer's entry from Timer.get_stats()."""
...
class HealthMonitorService(CliffracerService):
    """
    ...
    def __init__(self):
        config = ServiceConfig(name="health_monitor_service")
    ...
    async def health_check(self):
        ...
        try:
            ...
            print(f"[OK] Health check #{self.health_check_count} completed - Status: healthy")
            ...
            print(f"[ERROR] Health check #{self.health_check_count} failed: {e}")
    ...
    async def collect_metrics(self):
        ...
        print(f"[METRICS] Metrics collected #{self.metrics_collected}: {current_metrics}")
    ...
    async def cleanup_old_data(self):
        ...
        print(f"[CLEAN] Cleanup #{self.cleanup_count}: Removed {len(old_keys)} old metric entries")
    ...
    async def sync_data(self):
        ...
        try:
            ...
            print(f"[INFO] Data sync #{self.sync_count} completed: {sync_result}")
            ...
            print(f"[ERROR] Data sync #{self.sync_count} failed: {e}")
    ...
    def connection_heartbeat(self):
        ...
        print(f"[INFO] Heartbeat - Active connections: {self.database_connections}")
    ...
    async def get_health_status(self) -> HealthStatus:
        ...
        return HealthStatus(**self.health_status)
    ...
    @rpc
    async def get_timer_statistics(self) -> TimerStatistics:
        """Get timer execution statistics"""
        return TimerStatistics(**self.get_timer_stats())

    @rpc
    async def get_recent_metrics(self, limit: int = 5) -> dict[str, MetricsSample]:
...
async def main():
    ...
    Main example runner
    """
    print("[INFO] Cliffracer Timer Example")
    ...
    try:
        print("[INFO] Starting Health Monitor Service...")
        await service.start()

        print("[OK] Service started! Timers are now running...")
        ...
        print("\n[METRICS] Final Timer Statistics:")
