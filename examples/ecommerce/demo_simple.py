...
class InMemoryMessageBus:
    ...
    async def publish(self, subject: str, data: dict):
        ...
        print(
            f"[INFO] NATS: {subject} -> {len(self.subscribers.get(subject, []))} subscribers ({latency * 1000:.2f}ms)"
        )
...
class InventoryService:
    ...
    async def on_order_created(self, data: dict):
        ...
        if can_reserve:
            ...
            await message_bus.publish(
                "inventory.reserved", {"order_id": order_id, "total_amount": data["total_amount"]}
            )
        else:
            self.logger.error(
                "Insufficient inventory", order_id=order_id, action="inventory_insufficient"
            )
...
class PaymentService:
    ...
    async def on_inventory_reserved(self, data: dict):
        ...
        await message_bus.publish(
            "payment.completed",
            {"payment_id": payment_id, "order_id": order_id, "amount": amount, "success": success},
        )
...
class MonitoringService:
    ...
    async def show_metrics(self, services: dict):
        ...
        print("\n" + "=" * 60)
        print("[METRICS] CLIFFRACER SYSTEM METRICS")
        print("=" * 60)
        print(f"[INFO]  System Uptime: {uptime:.1f}s")
        print(f"[INFO] Messages Processed: {bus_stats['messages_processed']}")
        print(f"[INFO] Avg Message Latency: {bus_stats['average_latency_ms']:.3f}ms")
        print(f"[INFO] Active Subscriptions: {bus_stats['active_subscriptions']}")

        print("\n[INFO] ORDER SERVICE:")
        order_metrics = services["order"].metrics
        print(f"   Orders Created: {order_metrics['orders_created']}")
        if order_metrics["processing_times"]:
            avg_time = sum(order_metrics["processing_times"]) / len(
                order_metrics["processing_times"]
            )
...
async def generate_orders(order_service: OrderService):
    ...
    while True:
        try:
            ...
            print(f"[ERROR] Error generating order: {e}")
            await asyncio.sleep(5)


async def main():
    """Run the simplified demo"""
    print("[INFO] Cliffracer NATS Framework - Core Concepts Demo")
    print("=" * 60)
    print()
    print("This demo showcases:")
    print("  [OK] Multi-service event-driven architecture")
    print("  [OK] Sub-millisecond message routing")
    print("  [OK] Structured JSON logging")
    print("  [OK] Type-safe service communication")
    print("  [OK] Real-time metrics and monitoring")
    print()
    print("[INFO] Watch the structured logs and metrics!")
    print("[NOTE] Each service processes events in real-time")
    print()
    ...
    try:
        ...
        print("\n[NOTE] Key Takeaways:")
        print("  [INFO] Sub-millisecond message routing between services")
        print("  [INFO] Structured logging makes debugging easy")
        print("  [INFO] Event-driven architecture scales naturally")
        print("  [METRICS] Built-in monitoring shows real business metrics")
        print("  [INFO]  Type safety prevents runtime errors")

        print("\n[INFO] This is just the core - full version includes:")
        print("  [METRICS] Zabbix dashboards with advanced metrics")
        print("  [INFO] HTTP APIs with FastAPI integration")
        print("  [INFO] Production Docker deployment")
        print("  [INFO]  AWS cloud integration")
        print("  [INFO] Authentication and authorization")
