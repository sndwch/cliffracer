...
async def test_order_creation():
    ...
    print("[INFO] Testing Order Creation API")
    print("-" * 40)
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        print(f"[OK] Order #{i + 1} created successfully!")
                        ...
                        print(f"[ERROR] Order #{i + 1} failed: {response.status} - {error_text}")
                        print()

            except Exception as e:
                print(f"[ERROR] Order #{i + 1} error: {e}")
                print()
...
async def test_order_retrieval():
    ...
    print("[INFO] Testing Order Retrieval API")
    print("-" * 40)
        ...
        try:
            ...
            async with session.get("http://localhost:8001/orders") as response:
                if response.status == 200:
                    ...
                    print(f"[METRICS] Found {len(orders)} total orders in system")

                    if orders:
                        # Show details of the latest order
                        latest_order = orders[-1]
                        print("\n[INFO] Latest Order Details:")
                        ...
                        async with session.get(
                            ...
                            if detail_response.status == 200:
                                ...
                                print("[OK] Individual order retrieval successful")
                                print(f"   Items: {len(order_detail['items'])}")
                            else:
                                print(
                                    f"[ERROR] Individual order retrieval failed: {detail_response.status}"
                                )
                    else:
                        print("   No orders found - system might be starting up")

                else:
                    print(f"[ERROR] Failed to retrieve orders: {response.status}")

        except Exception as e:
            print(f"[ERROR] Order retrieval error: {e}")
...
async def test_api_documentation():
    ...
    print("[INFO] Testing API Documentation")
    print("-" * 40)
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get("http://localhost:8001/docs") as response:
                if response.status == 200:
                    print("[OK] API documentation is accessible at http://localhost:8001/docs")
                    print("   You can test the API interactively using the Swagger UI")
                else:
                    print(f"[ERROR] API documentation not accessible: {response.status}")
...
async def monitor_system_metrics():
    ...
    print("[METRICS] System Monitoring Information")
    print("-" * 40)

    print("[INFO] Available Endpoints:")
    print("   [METRICS] NATS Monitoring: http://localhost:8222")
    print("   [INFO] Order API Docs: http://localhost:8001/docs")
    print()

    print("[INFO] Log Monitoring:")
    ...
    print("[INFO] Key Metrics to Watch in Zabbix:")
...
async def main():
    """Run all tests"""
    print("[INFO] Cliffracer Live System Testing")
    print("=" * 50)
    print()
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get("http://localhost:8001/docs") as response:
                if response.status != 200:
                    print("[ERROR] Order service is not running at http://localhost:8001")
                    print("   Please start the system with: ./setup_live_demo.sh")
                    return
        except Exception:
            print("[ERROR] Cannot connect to order service at http://localhost:8001")
            print("   Please start the system with: ./setup_live_demo.sh")
            return

    print("[OK] System appears to be running, starting tests...\n")
    ...
    print()
    print("[NOTE] Tips for monitoring:")
