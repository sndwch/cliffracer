...
# RPC response models. Pydantic models serialize complex types such as
# datetime and Decimal to JSON-compatible formats during dispatch.
...
class MonitoredService(CliffracerService):
    """The two kinds of number every service below reports.

    `MetricsExtension` counts, times and separates the errors and refusals of
    every dispatch, and puts them on `/health` under the attribute name --
    that is the framework's own measurement, and the declaration is all of the
    code for it. An extension declared on a base class is inherited: services
    are collected from each class's `__dict__` walking the MRO, so the five
    services below get this one without repeating it.

    `record_metric` logs business metrics (such as order totals and processing times)
    as structured logs.
    """
    ...
    async def record_metric(
        ...
        """Emit one business metric as a structured log line."""
        self.logger.info(
            f"metric {name}={value}",
            extra={"metric": name, "value": value, "tags": tags or {}, "action": "metric"},
        )
...
class OrderService(MonitoredService):
    ...
    async def on_startup(self):
        ...
        self.logger.info(
            "OrderService starting up", extra={"service": "order_service", "version": "1.0.0"}
        )

        # Record startup metric
        await self.record_metric(
            "service.startup", 1, {"service": "order_service", "version": "1.0.0"}
        )
    ...
    async def get_order(self, order_id: str) -> Order | None:
        ...
        processing_time = time.time() - start_time
        await self.record_metric("orders.retrieved", 1, {"found": "true" if order else "false"})
    ...
    async def update_order_status(self, order_id: str, new_status: OrderStatus):
        ...
        await self.broadcast_status_changed(order_id, old_status.value, new_status.value)
    ...
    @broadcast("orders.order_created")
    ...
    @broadcast("orders.order_status_changed")
    async def broadcast_status_changed(self, order_id: str, old_status: str, new_status: str):
        """Broadcast order status change event"""
        return OrderStatusChangedEvent(
            order_id=order_id,
            old_status=OrderStatus(old_status),
            new_status=OrderStatus(new_status),
            timestamp=datetime.now(UTC),
        )

    @validated_listener("payments.payment_completed", PaymentCompletedEvent, fanout=True)
...
class InventoryService(MonitoredService):
    """Inventory management service"""

    def __init__(self):
        config = ServiceConfig(
            name="inventory_service", description="Product inventory management", auto_restart=True
        )
        super().__init__(config)
    ...
    async def on_startup(self):
        """Service startup"""
        self.logger.info(
            "InventoryService starting up",
            extra={"service": "inventory_service", "total_products": len(self.inventory)},
        )

    @rpc
    async def check_availability(self, check: InventoryCheck) -> AvailabilityResult:
        ...
        if not product:
            result = AvailabilityResult(
                product_id=check.product_id, available=False, reason="product_not_found"
            )
        else:
        ...
        await self.record_metric(
            "inventory.check",
            1,
            {"product_id": check.product_id, "available": str(result.available).lower()},
        )
        await self.record_metric("inventory.check_time", processing_time)

        return result

    @validated_listener("orders.order_created", OrderCreatedEvent, fanout=True)
        ...
        try:
            ...
            if can_reserve_all:
                ...
                self.logger.warning(
                    ...
                    extra={"order_id": event.order_id, "action": "inventory_insufficient"},
                )
            ...
            self.logger.error(
                ...
                extra={"order_id": event.order_id, "error": str(e), "action": "reservation_failed"},
            )
    ...
    @broadcast("inventory.reserved")
    async def broadcast_inventory_reserved(
        self, order_id: str, items: list[OrderItem], reservation_id: str
    ):
        """Broadcast inventory reservation event"""
        return InventoryReservedEvent(order_id=order_id, items=items, reservation_id=reservation_id)


class PaymentService(MonitoredService):
    """Payment processing service"""
    ...
    async def on_startup(self):
        """Service startup"""
        self.logger.info("PaymentService starting up", extra={"service": "payment_service"})
    ...
    async def process_payment(self, request: PaymentRequest) -> PaymentResult:
        ...
        await self.broadcast_payment_completed(
            request.order_id, payment_id, float(request.amount), payment.status.value
        )

        await self.record_metric(
            "payments.processed",
            1,
            {"status": "success" if success else "failed", "method": request.payment_method},
        )
    ...
    @validated_listener("inventory.reserved", InventoryReservedEvent, fanout=True)
    async def on_inventory_reserved(self, event: InventoryReservedEvent):
        ...
        payment_request = PaymentRequest(
            order_id=event.order_id, amount=total_amount, payment_method="credit_card"
        )

        await self.process_payment(payment_request)

    @broadcast("payments.payment_completed")
    async def broadcast_payment_completed(
        self, order_id: str, payment_id: str, amount: float, status: str
    ):
...
class NotificationService(MonitoredService):
    ...
    async def on_startup(self):
        """Service startup"""
        self.logger.info(
            "NotificationService starting up", extra={"service": "notification_service"}
        )
    ...
    @validated_listener("orders.order_status_changed", OrderStatusChangedEvent, fanout=True)
...
class LoadGeneratorService(MonitoredService):
        ...
        while self.running:
            try:
                ...
                for _ in range(num_items):
                    ...
                    items.append(
                        OrderItem(product_id=product_id, name=name, quantity=quantity, price=price)
                    )
                ...
                # Create order via RPC using keyword arguments.
                order = await self.call_rpc(
                    "order_service",
                    "create_order",
                    request=order_request.model_dump(mode="json"),
                )
                ...
                self.logger.error(
                    "Load generation error",
                    extra={"error": str(e), "action": "load_generation_error"},
                )
...
async def main():
    ...
    LoggingConfig.configure(service_name="ecommerce", log_level="INFO")
