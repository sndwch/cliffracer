...
class OrderService(HTTPNATSService):
    ...
    async def broadcast_status_changed(
        self, order_id: str, old_status: OrderStatus, new_status: OrderStatus
    ):
...
class PaymentService(ValidatedNATSService):
    ...
    async def process_payment(self, request: PaymentRequest) -> dict:
        ...
        await self.broadcast_payment_completed(
            request.order_id, payment_id, request.amount, payment["status"]
        )
    ...
    async def broadcast_payment_completed(
        self, order_id: str, payment_id: str, amount: Decimal, status: PaymentStatus
    ):
        """Broadcast payment completion event"""
        return PaymentCompletedEvent(
            order_id=order_id, payment_id=payment_id, amount=amount, status=status
        )
