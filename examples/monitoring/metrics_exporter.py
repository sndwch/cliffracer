...
class MetricsCollector:
    ...
    def get_metrics(self):
        ...
        # Calculate averages
        avg_latency = sum(self.metrics['message_latencies']) / len(self.metrics['message_latencies']) if self.metrics['message_latencies'] else 0
        avg_order_value = sum(self.metrics['order_amounts']) / len(self.metrics['order_amounts']) if self.metrics['order_amounts'] else 0
        
        # Payment success rate
        total_payments = self.metrics['payments_processed'] + self.metrics['payments_failed']
        payment_success_rate = (self.metrics['payments_processed'] / total_payments * 100) if total_payments > 0 else 0
