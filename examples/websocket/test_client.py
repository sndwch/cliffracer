...
async def websocket_listener():
    ...
    print(f"[INFO] Connecting to WebSocket at {uri}...")
        ...
        async with websockets.connect(uri) as websocket:
            ...
            async for message in websocket:
                ...
                if data.get("type") == "connection":
                    ...
                    print(f"[{timestamp}] [INFO] Pong received")
                elif data.get("type") == "notification":
                    notif = data["data"]
                    print(f"[{timestamp}] [INFO] Notification: {notif['title']}")
                    print(f"    User: {notif['user_id']}")
                    print(f"    Message: {notif['message']}")
                    print(f"    Type: {notif['notification_type']}")
                elif data.get("type") == "broadcast":
                    # BroadcastMessages are automatically relayed
                    print(f"[{timestamp}] [INFO] Broadcast: {data['data']['source_service']}")
                    print(f"    Data: {json.dumps(data['data'], indent=2)}")
                else:
                    print(f"[{timestamp}] [INFO] {json.dumps(data, indent=2)}")
...
async def send_test_notifications():
    ...
    for i, notif in enumerate(notifications, 1):
        print(f"\n[INFO] Sending notification {i}/{len(notifications)}...")
        ...
        try:
            ...
            print(f"[OK] Sent! Notification ID: {result['result']['notification_id']}")
            print(
...
async def main():
    ...
    print("[INFO] WebSocket Notification Test Client")
    print("=" * 50)
