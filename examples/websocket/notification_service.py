...
class NotificationSent(BaseModel):
    """What send_notification answers its RPC caller."""
...
class NotificationService(CliffracerService):
    ...
    async def on_startup(self):
        ...
        self.logger.info(
            ...
            extra={"service": "notification_service", "port": self.http.port},
        )
    ...
    # Payload arguments are validated directly from handler parameter annotations.
    @rpc
    ...
    # @validated_listener, not @listener. This read
    # `@listener(NotificationMessage, fanout=True)`, passing a model class
    # where a SUBJECT STRING belongs -- which the decorator refuses by name,
    # pointing at this one.
    @validated_listener("notifications.broadcast", NotificationMessage, fanout=True)
    async def on_notification_broadcast(self, message: NotificationMessage):
        """
        Listen for notifications from other services.

        This service relays broadcasts to its own websocket clients, so this
        handler is only for logging.
        """
        self.logger.info(
...
async def main():
    ...
    print("[INFO] Starting WebSocket-Enabled Notification Service")
    print("=" * 50)
    ...
    try:
        ...
        print("\n[STOP] Shutting down notification service...")
        await service.stop()
