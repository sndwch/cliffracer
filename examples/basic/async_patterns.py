...
class OrderNATSService(CliffracerService):
    ...
    @rpc
    async def create_order(self, customer_id: str, items: list[OrderItem], total: float) -> Order:
...
class InventoryService(CliffracerService):
    ...
    async def check_availability(self, items: list[OrderItem]) -> dict[str, ItemAvailability]:
        ...
        for item in items:
            ...
            results[name] = ItemAvailability(
                requested=quantity, available=available, in_stock=available >= quantity
            )

        return results

    @rpc
    async def reserve_items(self, order_id: str, items: list[OrderItem]) -> Reservation:
        ...
        return Reservation(order_id=order_id, reserved=reserved)

    @async_rpc
    async def restock_item(self, item_name: str, quantity: int) -> None:
...
class NotificationService(CliffracerService):
    ...
    @async_rpc
    async def inventory_reserved(self, order_id: str, items: dict[str, int]) -> None:
        """Handle inventory reservation notifications"""
        print(f"Notification: Items reserved for order {order_id}: {items}")

    # fanout=True ensures notifications are broadcast to console handlers.
    @event_handler("orders.status_updated", fanout=True)
    async def on_order_status_updated(
        self, subject: str, order_id: str = "", status: str = ""
    ) -> None:
        """React to order status updates"""
        print(f"Notification: Order {order_id} status changed to {status}")

    @event_handler("inventory.restocked", fanout=True)
    async def on_inventory_restocked(
        self, subject: str, item_name: str = "", quantity: int = 0, new_total: int = 0
    ) -> None:
...
def run_demo():
    ...
    LoggingConfig.configure(service_name="async_patterns")
