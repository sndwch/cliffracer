...
class SimpleTestService:
    ...
    async def handle_ping(self, msg):
        ...
        response = {"pong": True, "timestamp": time.time(), "request_id": self.request_count}
    ...
    async def handle_echo(self, msg):
        ...
        try:
            ...
            response = {"echo": data, "timestamp": time.time(), "request_id": self.request_count}
        except Exception as e:
            response = {"error": str(e), "timestamp": time.time(), "request_id": self.request_count}
        await msg.respond(json.dumps(response).encode())
