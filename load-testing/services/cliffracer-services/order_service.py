...
from shared.models import (
    ComplexOrder, OrderItem, CustomerProfile, OrderStatus,
...
class OrderProcessingService(ValidatedNATSService):
    ...
    async def process_order(self, order: ComplexOrder) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                "order_id": order.order_id,
                "status": "processed",
                "estimated_ship_date": order.expected_ship_date.isoformat() if order.expected_ship_date else None,
    ...
    # @validated_rpc  # Simplified for load testing
    async def large_payload_processing(self, batch_request: BatchProcessingRequest) -> Dict[str, any]:
        """
        ...
        return {
            ...
            "memory_efficiency_items_per_mb": processed_items / (total_size_bytes / (1024 * 1024)) if total_size_bytes > 0 else 0
        }
    ...
    async def _validate_business_rules(self, order: ComplexOrder) -> Dict[str, any]:
        ...
        if (order.customer.customer_id not in self.customer_cache and 
        ...
        if order.shipping.shipping_address.country != "US" and order.total_amount > Decimal("10000.00"):
