...
class PaymentDetails(BaseModel):
    ...
    cardholder_name: str | None = Field(None, max_length=100)
...
class ComplexOrder(BaseModel):
    ...
    def validate_total_amount(cls, v, values):
        ...
        if all(
            key in values for key in ["subtotal", "tax_amount", "shipping_cost", "discount_amount"]
        ):
            expected = (
                values["subtotal"]
                + values["tax_amount"]
    ...
    def validate_items_subtotal(cls, v, values):
        ...
        if v and "subtotal" in values:
            ...
            if abs(items_total - values["subtotal"]) > Decimal("0.01"):
                raise ValueError(
                    f"Items total {items_total} doesn't match subtotal {values['subtotal']}"
                )
        return v
...
class ValidationErrorTest(BaseModel):
    ...
    complex_pattern: str = Field(
        ..., pattern=r"^TEST-\d{4}-[A-Z]{3}$"
    )  # Will fail with wrong pattern
