...
class DataGenerator:
    ...
    def generate_address(self, residential: bool = True) -> Address:
        ...
        return Address(
            ...
            delivery_instructions=self.fake.text(max_nb_chars=200)
            if random.random() < 0.2
            else None,
        )

    def generate_customer_profile(self, customer_id: str | None = None) -> CustomerProfile:
        ...
        preferences = {
            ...
            "special_instructions": self.fake.text(max_nb_chars=100)
    ...
    def generate_product(self, product_id: str | None = None) -> Product:
        ...
        variants = [
            self.generate_product_variant(product_name) for _ in range(random.randint(1, 5))
        ]
        ...
        # Generate image URLs
        images = [
            f"https://example.com/images/{product_id}/{i}.jpg" for i in range(random.randint(1, 6))
        ]
        ...
        metadata = {
            ...
            "compliance_certifications": random.sample(
                ["CE", "FCC", "UL", "ISO9001", "RoHS", "ENERGY_STAR"], k=random.randint(0, 3)
            ),
            "technical_specs": {
                "power_consumption": f"{random.randint(5, 500)}W"
                if random.random() < 0.5
                else None,
                "operating_temperature": f"{random.randint(-10, 50)}°C to {random.randint(60, 85)}°C"
                if random.random() < 0.3
                else None,
                "connectivity": random.sample(
                    ["WiFi", "Bluetooth", "USB", "Ethernet"], k=random.randint(0, 2)
                ),
            },
        }
    ...
    def generate_order_item(self, product: Product | None = None) -> OrderItem:
        ...
        return OrderItem(
            ...
            special_instructions=self.fake.text(max_nb_chars=200)
    ...
    def generate_payment_details(self, amount: Decimal) -> PaymentDetails:
        ...
        if method in [PaymentMethod.CREDIT_CARD, PaymentMethod.DEBIT_CARD]:
            ...
            payment.card_brand = random.choice(
    ...
    def generate_shipping_details(self, address: Address, total_weight: int) -> ShippingDetails:
        ...
        return ShippingDetails(
            ...
            tracking_number=f"{carrier[:3].upper()}{random.randint(10000000, 99999999)}"
            ...
            else Decimal(str(random.randint(50, 500))),
            signature_required=random.random() < 0.3,
        )

    def generate_complex_order(
        self,
        ...
        # Generate shipping
        total_weight = sum(
            item.product_snapshot.variants[0].weight_grams * item.quantity for item in items
        )
        ...
        if force_error or (random.random() < self.error_rate):
            ...
            if error_type == "negative_total":
                ...
                customer.date_of_birth = datetime.now(UTC) + timedelta(
                    days=365
                )  # Future birth date
        ...
        return ComplexOrder(
            ...
            promotional_codes=[
                f"PROMO{random.randint(10, 99)}" for _ in range(random.randint(0, 2))
            ],
    ...
    def generate_analytics_event(self) -> AnalyticsEvent:
        ...
        properties = {
            ...
            "tags": random.sample(
                ["mobile", "desktop", "tablet", "ios", "android", "chrome", "firefox"],
            ...
            items.append(
                {
                    ...
                    "data": {
                        ...
                        "value": random.uniform(0, 1000),
                        "tags": random.sample(
                            ["tag1", "tag2", "tag3", "tag4", "tag5"], k=random.randint(0, 3)
                        ),
                        "nested_data": {
                            "level1": {
                                "level2": {
