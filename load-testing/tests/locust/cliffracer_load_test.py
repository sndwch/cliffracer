...
class OrderProcessingUser(NATSUser):
    ...
    def process_single_order(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call("order_processing_service.rpc.process_order", order_data)
        )
    ...
    def process_order_batch(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call(
                "order_processing_service.rpc.batch_process_orders",
                orders_data,
    ...
    def get_service_metrics(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call("order_processing_service.rpc.get_service_metrics", {})
        )
...
class AnalyticsUser(NATSUser):
    ...
    def send_analytics_events(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call(
                "order_processing_service.rpc.analytics_ingestion", events_data, timeout=3.0
            )
        )
...
class LargePayloadUser(NATSUser):
    ...
    def process_large_batch(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call(
                "order_processing_service.rpc.large_payload_processing",
                batch_data,
...
class ErrorScenarioUser(NATSUser):
    ...
    def trigger_validation_errors(self):
        ...
        response = self.run_async_task(
            self.nats_rpc_call(
                "order_processing_service.rpc.error_scenario_test", error_data, timeout=2.0
            )
        )
...
class MixedWorkloadUser(NATSUser):
    ...
    def mixed_analytics_load(self):
        ...
        _ = self.run_async_task(
            self.nats_rpc_call("order_processing_service.rpc.analytics_ingestion", events_data)
        )
    ...
    def occasional_large_payload(self):
        ...
        _ = self.run_async_task(
            self.nats_rpc_call(
                "order_processing_service.rpc.large_payload_processing", batch_data, timeout=20.0
            )
        )
